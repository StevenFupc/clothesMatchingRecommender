...
module.exports = {
	getBottomAll: function (data) {
		return db.execute({
			sql: 'SELECT id as "id", product_name as "Product Name" FROM adidas.Bottom order by id DESC ',
			//sql : 'SELECT * FROM "SAP_HHP_MEDEX"."sap.hhp.medexplorer.db::SV_ANSWERS_ALL"',
			cb: data.cb
		});
	},

	getBottomById: function (data) {
		return db.execute({
			sql: 'SELECT id as "id", product_name as "Product Name" FROM adidas.Bottom'
			+ ' WHERE id = ?',
			cb: data.cb,
			param: data.param
		});
	...
	deleteBottomById: function (data) {
		return db.execute({
